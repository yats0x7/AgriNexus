...
const generateWeatherData = (location: string): WeatherData => ({
    ...
    alerts: [
        {
            type: 'Heavy Rainfall',
            message: `Heavy to very heavy rainfall expected in ${location} on 21-22 Dec`,
            severity: 'high',
            icon: <CloudRain className="h-4 w-4" />,
            advisory: ['Avoid irrigation', 'Provide proper drainage', 'Harvest mature crops']
        },
        {
            type: 'Thunderstorm',
            message: 'Thunderstorms with gusty winds likely next Monday',
            severity: 'moderate',
            icon: <CloudLightning className="h-4 w-4" />,
            advisory: ['Secure farm equipment', 'Avoid field operations', 'Monitor livestock']
        }
    ]
});
...
export default function Weather() {
    ...
    const fetchWeatherData = React.useCallback(async () => {
        ...
        try {
            const weatherAlerts = await WeatherAlert.filter({
    ...
    const getCurrentIcon = (condition: string): JSX.Element => {
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <Card className="shadow-lg">
                                ...
                                <CardContent className="space-y-4">
                                    {[...weather.alerts, ...alerts].map((alert, index) => (
                                    ...
                                    {weather.alerts.length === 0 && alerts.length === 0 && (
                                        <Alert>
                                            <div className="flex items-start gap-2">
